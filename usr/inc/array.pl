# array.pl generated from libpl-0.3f/src/array.pl Mar 2025 
{ # PERL ARRAYS SUPPORT
...
our sub max {
  my ($max,@arr) = @_;
  $max=$_>$max?$_:$max for @arr;
  return $max }

# return 1 if all elements in array are the same
our sub allsame {
  my ($first,@arr) = @_;
  for(@arr) { return 0 if $_ != $first }
  return 1 }
